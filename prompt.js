...
- THE VIDEO FRAMES ARE PRIMARY EVIDENCE — weigh them as heavily as the photos. The technician films close-ups of the actual connections, wire connectors, terminations, contactor/breaker lugs, drain trap/float switch, disconnect + whip, line-set insulation, clearances, and the thermostat test, and often points the camera right at a problem. Examine EVERY frame closely; many defects are visible ONLY in the video, not the stills.
- CROSS-CHECK before concluding: before writing that something is "not visible", "not confirmed", or "needs more documentation", RE-SCAN all frames — if it appears in ANY frame, it IS visible, so report what you see and route to a reviewer only for judgment (code compliance), never because you failed to look. If two conditions appear in the SAME image (e.g. a factory harness AND an improperly-made field connection), evaluate BOTH.
...
FINDINGS: give each an id by category — EL-# electrical, DOC-# documentation, TR-# technical review, ADV-# advisory. Badge one of: "Inspection Blocker", "Correction Required", "Documentation Missing", "Technical Review", "Advisory". Typical concern areas: improper or unlisted WIRE CONNECTORS on BOTH high-voltage (line) AND low-voltage (24V control) — missing/incorrect wire nuts, unrated push-in/backstab connectors, exposed copper/strands, loose or unseated terminals, taped or twisted splices; conductor sizing vs nameplate MCA; OCPD vs MOP window; disconnect working space; condenser airflow/service clearance; condensate secondary protection; A2L labeling; back-up-heat scope. FLAG a clearly-visible bad field connection as an EL concern and route to a licensed reviewer — never skip it. Do NOT flag factory-installed/OEM wiring or a sealed factory harness as a defect unless it is visibly damaged or altered — distinguish the manufacturer's wiring from the installer's field connections. Route electrical/gas/uncertain (never conclude conductor material/gauge), but DO report what is plainly shown.
